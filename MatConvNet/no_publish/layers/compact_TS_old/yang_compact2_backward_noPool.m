function pre=yang_compact2_backward_noPool(layer, pre, now)  
    bsn=8;
    ...
    for img=1:ceil(n/bsn)
        ...
        temp=now.dzdx(:,:,:,interSmall);
        temp=permute(temp, [3,1,2,4]);
        temp=reshape(temp, outDim, []);
        temp=fft(temp,[], 1);
        
        [out(:,interLarge), t_dzdw]=aBatch(temp, gones, ch, cs, ...
...
function [out, dzdw]=aBatch(repFftDzdy, gones, ch, cs, x, gpuMode, outDim, h, w, c, n0, ia, comp, learnW)
    if gpuMode

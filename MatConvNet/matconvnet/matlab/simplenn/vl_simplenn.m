...
for i=1:n
  ...
  res(i).time = tic ;
  % debug yang
  %fprintf('forward layer %d, type: %s \n', i, l.type);
  ...
  switch l.type
    ...
    case 'loss'
      res(i+1).x = vl_nnloss(res(i).x, l.class, [], 'loss', l.lossType) ;
    case 'softmaxloss'
  ...
  % conserveMemory==forget things
  forget = opts.conserveMemory ;
  % if not a ReLU and during derivative, then don't forget
  forget = forget & (~doder || strcmp(l.type, 'relu')) ;
  % if a loss layer or a softmax loss layer, then don't forget
  forget = forget & ~(strcmp(l.type, 'loss') || strcmp(l.type, 'softmaxloss')) ;
  % if has field rememberOutput and its value is 1, then don't forget
  forget = forget & (~isfield(l, 'rememberOutput') || ~l.rememberOutput) ;
  % TOTAL EFFECT: when forget, don't forget at the time of:
  %     want to do derivative, and current is not relu
  %     final loss layer
  %     this layer has rememberOutput set to 1
  % Forget is all about current layer's input, not the output
  if forget
    res(i).x = [] ;
  end
...
if doder
  ...
  for i=n:-1:max(1, n-opts.backPropDepth+1)
    l = net.layers{i} ;
    % debug yang
    %fprintf('backward layer %d, type: %s \n', i, l.type);
    ...
    switch l.type
      ...
      case 'loss'
        res(i).dzdx = vl_nnloss(res(i).x, l.class, res(i+1).dzdx, 'loss', l.lossType) ;
      case 'softmaxloss'

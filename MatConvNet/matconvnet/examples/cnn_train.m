...
for epoch=start+1:opts.numEpochs
  ...
  if numGpus <= 1
    ...
    nowEval=(mod(epoch, opts.valInter)==0);
    if nowEval
  ...
  % change save to savefast
  if ~evaluateMode && (mod(epoch, opts.saveInter)==0)
      savefast(modelPath(epoch), 'net', 'info') ; 
  end
...
for t=1:opts.batchSize:numel(subset)
  ...
  for s=1:opts.numSubBatches
    ...
    debug_get_predictions=0;
    if debug_get_predictions && training
        fprintf('skip iter due to debug get predictions');
        return
    end
    ...
    if debug_get_predictions
        fprintf('*************getting predictions for Lisa********************');
        predictions = gather(res(end-1).x) ;
        [~,predictions] = sort(predictions, 3, 'descend') ;
        predictions=squeeze(predictions); % the shape is #classes*#batchsize
        global lisa_out
        if isempty(lisa_out)
            lisa_out=predictions;
        else
            lisa_out=cat(2, lisa_out, predictions);
        end
    end
    
% debug save an intermediate network and res
debug_weights=0;
if debug_weights
    %fprintf('gathering vars from GPU\n');
    global gnet gres gnetb gresb
    %gnetb=gnet;
    %gresb=gres;
    gres=res;
    %gnet=net;
    
    if any(gather(isnan(net.layers{1}.weights{1})))
        vl_simplenn_diagnose(gnet, gres);
        %fprintf('saving files\n')
        %savefast('data/exp_yang_fv/yang_debug_weights','res','net','im');

        error('debuging the weights. var saved. interupting');
    end
end
debug_getAnOutput=0;
if debug_getAnOutput && training
    diff2=mean(abs(res(1).dzdw{1}(:)));
    if diff2>10
        global gres gnet initNet
        gres=res;
        gnet=vl_simplenn_move(net,'cpu');
        
        vl_simplenn_diagnose(net, res);
        %diff1=compareNet(net, initNet);
        error('found');
        %savefast('data/exp_yang_fv/yang_debug_weights','cres','cnet');
    end
    fprintf('has analyze a batch, diff2=%f \n', diff2);
    %error('debuging to extract the activations. interupting');
end
...
for l=numel(net.layers):-1:1
  ...
  if 0
  if tempThisLR>1e-8
      fprintf('\n \t SGD at layer %d, LR %f, name %s \n',...
              l, tempThisLR, net.layers{l}.name);
  end
  end
end

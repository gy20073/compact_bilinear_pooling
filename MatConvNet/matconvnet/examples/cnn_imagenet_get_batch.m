...
% yang change it to GPU
imo = zeros(opts.imageSize(1), opts.imageSize(2), 3, ...
            numel(images)*opts.numAugments, 'single', 'gpuArray') ;
...
for i=1:numel(images)
  ...
  if any(abs(factor - 1) > 0.0001)
    % yang, not support bilinear rescale
    %imt = imresize(imt, ...
                   %'scale', factor,...
                   %'method', opts.interpolation) ;
    % image rescale on GPU
...
%%%%%%%% a better version of the preproessing.


function imo = cnn_imagenet_get_batch_bak(images, varargin)
fprintf('WARNING get_batch no augmentation yet\n')
...
for i=1:numel(images)
  ...
  offset = opts.averageImage ;
  % change to 3 numbers
  offset = reshape( mean(mean(offset, 1),2), 1,1, 3);
  
  %imo(:,:,:,si) = bsxfun(@minus, imt, offset) ;
  imo = bsxfun(@minus, imt, offset) ;
  imo=reshape(imo, size(imt,1), size(imt,2), 3, 1);

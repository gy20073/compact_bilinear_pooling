function yang_test_compact_bilinear_noPool()
    ...
    fwd=@(X)getfield(yang_compact_bilinear_RM_forward_nopool(...
                     struct('weights',{{w1,w2}}), struct('x', X), struct()), 'x');

    dzdy=rand(13,13,30,5,'single')+1;        
    dzdx=getfield(yang_compact_bilinear_RM_backward_nopool(...
    ...
    numerical_der_step=1e-2;
    fwd_w=@(w)getfield(yang_compact_bilinear_RM_forward_nopool(...
